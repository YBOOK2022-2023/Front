...
import {List,Avatar,IconButton,ListItem,ListItemAvatar, ListItemText, Button} from "@mui/material"
...
export default function SearchUser (props:any){
    ...
    //Ajout d'amis
    const AddFriend=useCallback(async (id:number)=>{
        const idToken = await getJwt();
        console.warn(idToken);
        return await configAxios.post(`/friendship`,{toID:id},
...
const SearchUser =useCallback(async (searchUser:string)=>{
    const idToken = await getJwt();
    console.warn(idToken);
    return await configAxios.get(`/usersearch/${searchUser}`,{
        headers:{Authorization:`Bearer ${idToken}`},
    }).then((response)=>{
    ...
    return(
        <div className="mt-4 m-2 c-cyan-500 " >
            <FormControl  margin="dense" fullWidth variant="standard">
             ...
             {okData==true && (<Button color="warning" onClick={handleClear}><ClearIcon/>Effacer Recherche</Button>)}
             <List>
                
             {searchData.map((user:any) => {
                return (
               
                <ListItem divider={true} key={user.id} secondaryAction={
                    <div className="flex flex-row">
                    <IconButton color="info" aria-label="add" onClick={()=>{setAddFriendid(user.id);handleAddFriend();}}>
