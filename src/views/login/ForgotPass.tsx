...
import { BoldLink, BoxContainer, FormContainer, Input, MutedLink, SpanAlert, SubmitButton } from './common';
...
export function ForgotPass (props:any){
    ...
    const { register, handleSubmit, formState: { errors } } = useForm<MyFieldsForgottenPass>({
  ...
  //onSubmit
    ...
    const resetPassword=()=>{
        ...
        getUser().confirmPassword(code,newpassword, {
            ...
            , onFailure: function (err) {
                ...
                }else if(err.message==="ExpiredCodeException: Invalid code provided, please request a code again."){
                    setErrorMessageComfirm("Code invalide ou  expiré, veuillez en demander un nouveau ou resaisir");
                }else if(err.message==="Attempt limit exceeded, please try after some time."){
                    setErrorMessageComfirm("Trop de tentatives, veuillez réessayer plus tard"); 
                }
                setOpenToastComfirm(true);
            }
        })
    ...
    return(
        <BoxContainer>
            ...
            <ValidationCodeDialog open={open} setOpen={setOpen} sendVerif={sendVerifForgotPass} setCode={setCode} code={code} resendCode={resetPassword} resendCodeText="Valider"/>
          ...
          {isErrorComfirm ? <Snackbar anchorOrigin={{vertical: 'top', horizontal: 'left' }}  open={openToastComfirm} autoHideDuration={6000} onClose={handleCloseToast}>
           <Alert onClose={handleCloseToastComfirm} severity='error'  sx={{ width: '100%' }} >
               {errorMessageComfirm}
           </Alert>
          </Snackbar> : <Snackbar anchorOrigin={{vertical: 'top', horizontal: 'left' }}  open={openToastComfirm} autoHideDuration={6000} onClose={handleCloseToast}>
              <Alert onClose={handleCloseToastComfirm} severity='success'  sx={{ width: '100%' }} >
                  {errorMessageComfirm}
              </Alert>
          </Snackbar>}
                ...
                <Input type="password" placeholder="Confirmer le mot de passe" {...register("comfirmPassword")} />
                {errors.email?.message && (<SpanAlert><HighlightOffTwoToneIcon></HighlightOffTwoToneIcon>{errors.email?.message}</SpanAlert>)}
                {errors.newpassword?.message && (<SpanAlert><HighlightOffTwoToneIcon></HighlightOffTwoToneIcon>{errors.newpassword?.message}</SpanAlert>)}
