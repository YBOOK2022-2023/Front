...
import { Search, SearchIconWrapper, StyledInputBase } from "../components/TopBar/SearchComponent";
...
function Friends() {
  ...
  const fetchFriends = useCallback(async () => {
    ...
    return await configAxios.get('/friendship', {
  ...
  const {isSuccess:queryUserSuccess,isError:queryUserError,isLoading:queryUserLoading,data:queryUserData}=useQuery('user', fetchFriends, { staleTime: 3600000 });
  if (queryUserLoading){ 
    return (
    <div><Box mt={"400px"} sx={{ display: 'flex' }}>
    <CircularProgress />
</Box>
</div>
)};
  ...
  queryUserData[0].friendships.map((friendlistData:{
    id:number,fromId:number,toId:number,status:string,
    from:{id:number,lastname:string,firstname:string,avatarS3Key:string},
    to:{avatarS3Key: string,id:number,lastname:string,firstname:string},
    friendInfo:{id:number,avatarS3Key:string,firstname:string,lastname:string}})=>{
      ...
      friendlist.current.push(
        {
          ...
          friendInfo: {id:friendlistData.from.id,
  ...
  queryUserData[0].friendshipPending.map((friendRequestData:{
    id:number,fromId:number,toId:number,status:string,
    from:{id:number,lastname:string,firstname:string,avatarS3Key:string},
    to:{avatarS3Key: string,id:number,lastname:string,firstname:string},
    friendInfo:{id:number,avatarS3Key:string,firstname:string,lastname:string}})=>{
      ...
      friendRequestReceived.current.push(
        {
          ...
          friendInfo: {id:friendRequestData.from.id,
...
console.log('riendRequestReceived: ',friendRequestReceived.current);
  ...
  if (queryUserError) {return <div>Une erreur est survenue</div>;}

  return (
    <div id='friends'>
      <InfoBar  showButton={false} userName={user.current?.lastname + ' ' + user.current?.firstname} numfriends={friendShipCount} />
      ...
      <Tabs key={0}
        ...
        icon2=""
        icon3=""
        content1={ <List key={1} sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper' }}>
          {friendlist.current.map((friend)=>(
         <div>
            <FriendList canChat={true} canIgnore={false} canAdd={false} key={friend.id} friend={friend}/>
         </div>
        )) } </List>}
        content2={<List key={2} sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper' }}>

          {
          friendRequestReceived.current.map((friendReceived)=>(
         <div>
            <FriendList canIgnore={true} canAdd={true} canChat={false} key={friendReceived.id} friend={friendReceived}/>
         </div>
       ))} </List>}
        content3={<List key={3} sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper' }}>

        {friendRequestSend.current.map((friendSend)=>(
       <div>
          <FriendList   canIgnore={true} canAdd={false} canChat={false} key={friendSend.id} friend={friendSend}/>
       </div>
     ))} </List>}
      />
    </div>
